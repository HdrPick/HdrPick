// JXL/JXR 集成 debug 构建脚本（离线资源版：清除 devUrl，内嵌 dist）
// 日志规则：启动时清空 tauri-build-jxl-hdr.log，之后完整累加全部输出
const { spawnSync } = require("node:child_process");
const fs = require("node:fs");
const path = require("node:path");

const LOG = path.join(__dirname, "tauri-build-jxl-hdr.log");
// 先清空日志
...
// 2. 前端构建（生成 dist：debug exe 的内嵌离线资源）
...
// 3. Rust debug 构建：
//    TAURI_CONFIG 覆盖 devUrl=null → tauri-build 嵌入 frontendDist(dist)
//    而非 devUrl(localhost:1420)，exe 可独立运行不依赖 vite dev server
if (results[0].status === 0) {
  const CARGO = "C:\\Users\\Administrator\\.cargo\\bin\\cargo.exe";
  results.push(
    run("rust-build", CARGO, ["build"], path.join(__dirname, "src-tauri"), {
...
//    bin 名为 hdr（Cargo.toml [[bin]]）；app.exe 为旧残留，优先 hdr.exe
const exeDir = path.join(__dirname, "src-tauri", "target", "x86_64-pc-windows-msvc", "debug");
const exe = ["hdr.exe", "app.exe"]
  .map((f) => path.join(exeDir, f))
  .find((p) => fs.existsSync(p) && fs.statSync(p).mtime > new Date(Date.now() - 3600_000));
